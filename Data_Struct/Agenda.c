...
int main(void) {
  ...
  m_lista=criar();
  t_elemento m_nome;
  int t,pos=0,i=0,pos_mostrar,search,pos_excluir,removido,escolha,menu,i_apagar,apagado;
  char dado[50],nome_apagar[50],nome_procurar[50];
  ...
  switch (menu){
    ...
    case 2:
      ...
      if (!strcmp(m_lista.vetor[i].nome,nome_apagar)){
        printf("%s apagado na posicao %d\n",nome_apagar,i);
        i_apagar=i;
        apagado = remover(&m_lista,i_apagar);
      }
      printf("\n");
      break;
    case 3:
      ...
      for(i = 0; i <= m_lista.n; i++)
      if (!strcmp(m_lista.vetor[i].nome,nome_procurar)){
         printf("\n%s esta na posicao %d:\nO nome é: [%s]\nO telefone1 e: [%s]\nO telefone2 e: [%s]\n\n", m_lista.vetor[i].nome,i,m_lista.vetor[i].nome,m_lista.vetor[i].telefone1,m_lista.vetor[i].telefone2);
      }
      break;
    case 4:
      // listar
